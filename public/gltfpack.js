/**This is an Emscripten-generated JavaScript wrapper for gltfpack, compiled to WebAssembly. It's NOT Three.js code – it's the binding layer that:
* Loads the WASM binary (gltfpack.wasm)
* Manages WebAssembly memory (HEAP8, HEAP32, etc.)
* Implements a virtual filesystem (the FS object) so gltfpack can read/write files in-memory
* Supports multithreading via Web Workers (pthreads) for parallel mesh processing
* Exposes gltfpack's CLI interface via callMain(args)
* Emscripten-compiled gltfpack (mesh optimizer)
* Runs with WebAssembly threads (pthreads) for parallel mesh processing

**/

async function createGltfpackModule(moduleArg = {}) {
	var moduleRtn;
	var Module = moduleArg;
	// Environment detection flags
	var ENVIRONMENT_IS_WEB = !!globalThis.window; //Running in browser main thread
	var ENVIRONMENT_IS_WORKER = !!globalThis.WorkerGlobalScope; //Running in a Web Worker
	var ENVIRONMENT_IS_NODE =
		globalThis.process?.versions?.node &&
		globalThis.process?.type != "renderer"; //Running in Node.js
	var ENVIRONMENT_IS_PTHREAD =
		ENVIRONMENT_IS_WORKER && self.name?.startsWith("em-pthread"); //Running as an Emscripten pthread worker thread
	var arguments_ = [];
	var thisProgram = "./this.program";
	var quit_ = (status, toThrow) => {
		throw toThrow;
	};
	var _scriptName = (function() { try { return new Function('return import.meta.url')(); } catch(e) { return typeof self !== 'undefined' ? self.location.href : ''; } })();
	var scriptDirectory = "";
	...
	if (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER) {
		try {
			scriptDirectory = new URL(".", _scriptName).href; //Set up script directory from URL
		} catch {}
		{
			if (ENVIRONMENT_IS_WORKER) {  //Synchronous binary file reader for workers (uses XHR)
				readBinary = (url) => {
	...
	var ABORT = false;
	var EXITSTATUS;
	//Helper to refresh memory views when WASM memory grows
	function growMemViews() {
		if (wasmMemory.buffer != HEAP8.buffer) {
			updateMemoryViews();
		}
	}
	//Promise handlers for async initialization
	...
	function findWasmBinary() {
		if (Module["locateFile"]) {
			return locateFile("gltfpack.wasm");
		}
		try { return new Function('return new URL("gltfpack.wasm", import.meta.url).href')(); } catch(e) { return "./gltfpack.wasm"; }
	}
	...
	var PThread = {
		...
		allocateUnusedWorker() {
			...
			if (Module["mainScriptUrlOrBlob"]) {
				...
				var workerUrl;
				try { workerUrl = new Function('return new URL("gltfpack.js", import.meta.url).href')(); } catch(e) { workerUrl = "./gltfpack.js"; }
	...
	function _fd_seek(fd, offset, whence, newOffset) {
		...
		try {
			...
			(growMemViews(), HEAP64)[(newOffset >>> 3) >>> 0] = BigInt(
				stream.position
			);
	...
	function assignWasmExports(wasmExports) {
		_main = Module["_main"] = wasmExports["Q"];
		__emscripten_tls_init = wasmExports["R"];
		_pthread_self = wasmExports["S"];
		_emscripten_builtin_memalign = wasmExports["T"];
		__emscripten_thread_init = wasmExports["U"];
		__emscripten_thread_crashed = wasmExports["V"];
		__emscripten_run_js_on_main_thread = wasmExports["W"];
		__emscripten_thread_free_data = wasmExports["X"];
		__emscripten_thread_exit = wasmExports["Y"];
		__emscripten_check_mailbox = wasmExports["Z"];
		_setThrew = wasmExports["_"];
		_emscripten_stack_set_limits = wasmExports["$"];
	...
	function applySignatureConversions(wasmExports) {
		...
		wasmExports["S"] = makeWrapper_p(wasmExports["S"]);
		wasmExports["T"] = makeWrapper_ppp(wasmExports["T"]);
		wasmExports["ba"] = makeWrapper_pp(wasmExports["ba"]);
		wasmExports["ca"] = makeWrapper_p(wasmExports["ca"]);
		return wasmExports;
	}
...
// Support both ES modules and classic scripts (workers use importScripts)
if (typeof globalThis !== 'undefined') globalThis.createGltfpack = createGltfpackModule;
if (typeof self !== 'undefined') self.createGltfpack = createGltfpackModule;
var isPthread = globalThis.self?.name?.startsWith("em-pthread");
isPthread && createGltfpackModule();
